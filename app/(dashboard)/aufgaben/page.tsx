...
export default function AufgabenPage() {
  ...
  const filtered =
    priorityFilter === "alle"
      ? mockAufgaben
      : mockAufgaben.filter((a) => a.prioritaet === priorityFilter);

  return (
    <div>
      <Header
        title="Aufgaben"
        subtitle={`${mockAufgaben.filter((a) => a.status !== "erledigt").length} offene Aufgaben`}
      />
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 rounded-xl border border-border bg-muted/40 p-1">
              <button
                ...
                className={`flex items-center justify-center w-8 h-7 rounded-lg transition-colors ${viewMode === "kanban" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground"}`}
              >
