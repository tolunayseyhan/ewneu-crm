...
import { mockAufgaben } from "@/lib/mock-data";
...
export default function AufgabenPage() {
  ...
  const filtered =
    priorityFilter === "alle"
      ? aufgaben
      : aufgaben.filter((a) => a.prioritaet === priorityFilter);

  const handleAdd = (aufgabe: Aufgabe) => {
    setAufgaben((prev) => [aufgabe, ...prev]);
  };
  ...
  return (
    <div>
      <Header
        title="Aufgaben"
        subtitle={`${aufgaben.filter((a) => a.status !== "erledigt").length} offene Aufgaben`}
      />
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 rounded-xl border border-border bg-muted/40 p-1">
              <button
                ...
                className={`flex items-center justify-center w-8 h-7 rounded-lg transition-colors ${viewMode === "kanban" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
              >
                    ...
                    <tr key={aufgabe.id} className="hover:bg-muted/30 transition-colors">
                      <td className="px-4 py-3.5">
                        ...
                        {aufgabe.beschreibung && (
                          <p className="text-xs text-muted-foreground line-clamp-1 mt-0.5">{aufgabe.beschreibung}</p>
                        )}
                      </td>
                      <td className="px-4 py-3.5 hidden md:table-cell">
                        <span className="text-xs text-muted-foreground">{aufgabe.kunde?.name1 || "–"}</span>
                      </td>
                      <td className="px-4 py-3.5 hidden lg:table-cell">
                        <span className="text-xs text-muted-foreground">{aufgabe.zugewiesener_name || "–"}</span>
                      </td>
